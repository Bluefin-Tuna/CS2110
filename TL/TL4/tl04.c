...
int stack_push(const char *question, enum category_t category) {
    if (!question || strlen(question) == 0) return FAILURE;
    struct post_t * p = malloc(sizeof(struct post_t));
    if (!p) return FAILURE;
    p->question = malloc(strlen(question) + 1);
    if (!p->question) { free(p); return FAILURE; }
    strcpy(p->question, question);
    p->category = category;
    if (numPosts + 1 >= capacity) {
        stack_arr = realloc(stack_arr, 2 * capacity * sizeof(numPosts));
        if (!stack_arr) { free(p->question); free(p); free(stack_arr); return FAILURE; }
        capacity *= 2;
    }
    *(stack_arr + numPosts++) = p;
    return SUCCESS;
}
...
int stack_pop(struct post_t *data) {
    if (!data || numPosts == 0) return FAILURE;
    struct post_t * p = *(stack_arr + --numPosts);
...
int destroy_stack(void) {
    struct post_t r;
    while (numPosts > 0) { stack_pop(&r); free(r.question); }
    free(stack_arr);
    return SUCCESS;
}