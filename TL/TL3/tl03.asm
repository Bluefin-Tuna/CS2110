...
SUMDIGITS
    ...
    AND R1, R1, 0  ;; Initialize sum

    AND R2, R2, 0  ;; Initialize R2 to counter

    ADD R3, R0, R2 ;; Getting currAddress to use
    LDR R3, R3, 0  ;; Getting char at currAddress

    LD R4, ASCIIOffset

    WHILE

        ADD R3, R3, 0   ;; Setting CC to check if curr == 0

        BRz END_WHILE

        ADD R3, R3, R4  ;; Getting decimal form of curr
        ADD R1, R1, R3  ;; Adding decimal form of curr to R1
        
        ADD R2, R2, 1   ;; Incrementing counter for address
        
        ADD R3, R0, R2  ;; Getting new currAddress to check
        LDR R3, R3, 0   ;; Loading new char at currAddress

        BR WHILE

    END_WHILE

    STR R1, R5, 3  ;; Saving sum as RV
...
DIGITALROOT
    ...
    LDR R0, R5, 4  ;; Storing num in R0

    LD R1, WRITE_SPACE ;; Storing WRITE_SPACE in R1
    LDR R1, R1, 0  ;; Getting address stored in WRITE_SPACE

    AND R2, R2, 0
    ADD R2, R2, -10

    ADD R6, R6, -2 ;; Making room for 2 additional args
    STR R0, R6, 1  ;; Storing R0 = num
    STR R1, R6, 0  ;; Storing R1 = address

    JSR TODECIMALSTRING

    ADD R6, R6, 3  ;; Clearing 3 args (address, num, RV) from stack

    ADD R6, R6, -1 ;; Making room for 1 additional arg
    STR R1, R6, 0  ;; Storing R1 = address

    JSR SUMDIGITS

    LDR R4, R6, 0  ;; Storing sum in R4
    ADD R6, R6, 2  ;; Clearing 2 args (address, RV) from stack

    IF

        ADD R3, R4, R2  ;; Setting CC to check if sum < 10

        BRn END_IF

        ADD R6, R6, -1  ;; Making room for 1 arg
        STR R4, R6, 0   ;; Storing R3 = sum

        JSR DIGITALROOT

        LDR R4, R6, 0   ;; Storing R3 = digital_root(sum)
        ADD R6, R6, 2   ;; Clearing 2 args (address, RV) from stack
