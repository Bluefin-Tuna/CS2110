...
metadata_t *find_right(metadata_t *freed_block) {
    ...
    while (c != NULL) {
        if ((uintptr_t) c == (uintptr_t) freed_block + TOTAL_METADATA_SIZE + freed_block->size) return c;
        c = c->next;
    }
    return NULL;
}
 ...
 */
 metadata_t *split_block(metadata_t *block, size_t size) {
    size_t t = size + TOTAL_METADATA_SIZE;
    metadata_t *s = (metadata_t*) ((uintptr_t) block + TOTAL_METADATA_SIZE + block->size - t);
    s->size = size;
    block->size = block->size - t;
    return s;
 }
 ...
 */
 metadata_t *find_best_fit(size_t size) {
    ...
    while (c != NULL) {
        ...
        if (c->size > size && (b == NULL || c->size < b->size)) b = c;
        c = c->next;
    }
    return b;
 }
...
void *my_malloc(size_t size) {
    ...
    metadata_t *b = find_best_fit(size);
    if (b == NULL) {
        metadata_t *n = my_sbrk(SBRK_SIZE);
        if (n == (void*) -1) {
            my_malloc_errno = OUT_OF_MEMORY;
            return NULL;
        }
        n->size = SBRK_SIZE - TOTAL_METADATA_SIZE;
        n->next = NULL;
        my_free((void*)((uintptr_t) n + TOTAL_METADATA_SIZE));
        return my_malloc(size);
    }
    if (b->size < size + MIN_BLOCK_SIZE) {
        remove_from_addr_list(b);
        return (void*) ((uintptr_t) b + TOTAL_METADATA_SIZE);
    }
    return (void*) ((uintptr_t) split_block(b, size) + TOTAL_METADATA_SIZE);
}
...
void my_free(void *ptr) {
    my_malloc_errno = NO_ERROR;
    if (ptr == NULL) return;
    metadata_t *s = (metadata_t*) ((uintptr_t) ptr - TOTAL_METADATA_SIZE);
    if (find_right(s) != NULL) {
        metadata_t * s1 = find_right(s);
        remove_from_addr_list(find_right(s));
        merge(s, s1);
    }
    metadata_t *s2 = find_left(s);
    if (s2 != NULL) {
        remove_from_addr_list(s2);
        merge(s2, s);
        s = s2;
    }
    add_to_addr_list(s);
...
void *my_realloc(void *ptr, size_t size) {
    ...
    size_t os = ((metadata_t*)((uintptr_t) ptr - TOTAL_METADATA_SIZE))->size;
    metadata_t *ns = my_malloc(size);
...
void *my_calloc(size_t nmemb, size_t size) {
    my_malloc_errno = NO_ERROR;
    metadata_t *s = my_malloc(nmemb * size);
